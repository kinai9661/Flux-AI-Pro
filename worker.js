...
:root {
    ...
    --primary-glow-light: rgba(250, 204, 21, 0.3);
    --primary-glow-subtle: rgba(250, 204, 21, 0.1);
    ...
    --secondary-glow-subtle: rgba(139, 92, 246, 0.1);
    ...
    --border-glow: rgba(250, 204, 21, 0.1);
    ...
    --shadow-glow-sm: 0 0 10px var(--primary-glow-subtle);
    ...
    --gradient-progress: linear-gradient(90deg, var(--primary), var(--primary-light));
...
.banana-loader {
    font-size: 60px;
    animation: spin-bounce 1.5s infinite;
    margin-bottom: 20px;
    filter: drop-shadow(0 0 20px var(--nano-glow));
}
.loading-text {
...
@media (max-width: 768px) {
    ...
    .main-stage { height: 45vh; }
    .logo-area { padding: var(--spacing-md) 0; }
    .logo-text { font-size: var(--font-2xl); }
    .quota-info { font-size: var(--font-sm); }
    .label-row { margin-bottom: var(--spacing-md); }
    .label-text { font-size: var(--font-sm); }
    .ratio-grid { gap: var(--spacing-sm); }
    .ratio-btn { padding: var(--spacing-sm); font-size: var(--font-xs); }
    .nano-drag-drop-zone { padding: 12px; min-height: 70px; }
    .nano-drag-drop-zone .drag-icon { font-size: 24px; }
    .nano-drag-drop-zone .drag-text { font-size: var(--font-xs); }
    .nano-drag-drop-zone .drag-subtext { font-size: 9px; }
    .history-dock { padding: var(--spacing-md); gap: var(--spacing-sm); }
    .history-item { width: 50px; height: 50px; }
    .lightbox img { max-width: 90%; max-height: 80vh; }
    .lightbox-close { top: 15px; right: 15px; font-size: 32px; }
    .lightbox-actions { flex-wrap: wrap; gap: 10px; }
    .action-btn { padding: 8px 16px; font-size: var(--font-sm); }
}
...
@media (max-width: 640px) {
    ...
    .logo-area { flex-direction: column; gap: var(--spacing-sm); }
    .logo-text { font-size: var(--font-xl); }
    .quota-info { flex-direction: column; gap: var(--spacing-xs); }
    .label-row { flex-direction: column; align-items: flex-start; gap: var(--spacing-xs); }
    .ratio-grid { grid-template-columns: repeat(4, 1fr); }
    .ratio-btn { padding: var(--spacing-xs); font-size: 10px; }
    .nano-drag-drop-zone { padding: 10px; min-height: 60px; }
    .nano-drag-drop-zone .drag-icon { font-size: 20px; }
    .nano-drag-drop-zone .drag-text { font-size: 11px; }
    .nano-drag-drop-zone .drag-subtext { font-size: 8px; }
    .history-dock { padding: var(--spacing-sm); gap: var(--spacing-xs); }
    .history-item { width: 45px; height: 45px; }
    .lightbox img { max-width: 95%; max-height: 75vh; }
    .lightbox-close { top: 10px; right: 10px; font-size: 28px; }
    .lightbox-actions { flex-direction: column; width: 100%; }
    .action-btn { width: 100%; justify-content: center; }
    .toast { padding: 10px 15px; font-size: 12px; top: 10px; right: 10px; }
}

@media (max-width: 480px) {
    body { font-size: 12px; }
    .sidebar { padding: var(--spacing-sm); }
    .main-stage { height: 35vh; }
    .logo-text { font-size: var(--font-lg); }
    .ratio-grid { grid-template-columns: repeat(3, 1fr); }
    .ratio-btn { padding: 6px; font-size: 9px; }
    .nano-drag-drop-zone { padding: 8px; min-height: 50px; }
    .nano-drag-drop-zone .drag-icon { font-size: 18px; }
    .nano-drag-drop-zone .drag-text { font-size: 10px; }
    .nano-drag-drop-zone .drag-subtext { font-size: 7px; }
    .history-dock { padding: var(--spacing-xs); gap: 4px; }
    .history-item { width: 40px; height: 40px; }
    .lightbox img { max-width: 98%; max-height: 70vh; }
    .lightbox-close { top: 8px; right: 8px; font-size: 24px; }
    .action-btn { padding: 6px 12px; font-size: 11px; }
}

@media (max-width: 320px) {
    .ratio-grid { grid-template-columns: repeat(2, 1fr); }
    .history-dock { display: none; }
}

/* Nano 頁面觸控優化 */
@media (hover: none) and (pointer: coarse) {
    .ratio-btn { min-height: var(--touch-target); min-width: var(--touch-target); }
    .nano-drag-drop-zone { min-height: calc(var(--touch-target) * 2); }
    .history-item { min-width: var(--touch-target); min-height: var(--touch-target); }
    .action-btn { min-height: var(--touch-target); }
}

/* Nano 頁面橫向模式優化 */
@media (max-height: 600px) and (orientation: landscape) {
    body { flex-direction: row; }
    .sidebar { width: 350px; height: 100vh; border-right: 1px solid var(--border); }
    .main-stage { height: 100vh; order: 1; border-bottom: none; }
    .history-dock { right: 10px; bottom: 10px; left: auto; }
}
...
@media (min-width: 1536px) {
    ...
    .ratio-btn { padding: var(--spacing-lg); font-size: var(--font-md); }
}
...
@keyframes loadingProgress { 0% { width: 0%; left: 0; } 50% { width: 70%; left: 30%; } 100% { width: 100%; left: 100%; } }
@keyframes stepPulse { 0%, 100% { transform: scale(1); opacity: 0.3; } 50% { transform: scale(1.5); opacity: 1; background: var(--nano-primary); } }
    ...
    const NanoDragDropHandler = {
        ...
        initDropZone(dropZoneId, fileInputId, onFileDrop) {
            ...
            dropZone.addEventListener('drop', (e) => {
                ...
                if (files.length > 0) {
                    this.handleFile(files[0], dropZone, onFileDrop);
                }
            }, false);
            ...
            fileInput.addEventListener('change', (e) => {
                if (e.target.files.length > 0) {
                    this.handleFile(e.target.files[0], dropZone, onFileDrop);
                }
            });
        },

        // 處理文件
        async handleFile(file, dropZone, onFileDrop) {
            // 驗證文件
            const validation = this.validateImageFile(file);
            if (!validation.valid) {
                this.showError(dropZone, validation.error);
                return;
            }
            ...
            // 顯示預覽
            await this.showPreview(dropZone, file);

            // 顯示文件信息
            this.showFileInfo(dropZone, file);
        ...
        // 顯示預覽
        async showPreview(dropZone, file) {
            try {
                const reader = new FileReader();
                reader.onload = (e) => {
                    ...
                    // 創建或更新預覽元素
                    let preview = dropZone.querySelector('.nano-drag-drop-preview');
                    if (!preview) {
                        preview = document.createElement('div');
                        preview.className = 'nano-drag-drop-preview';
        ...
        clearPreview(dropZone) {
            const preview = dropZone.querySelector('.nano-drag-drop-preview');
            if (preview) {
            ...
            const fileInfo = dropZone.querySelector('.drag-drop-file-info');
            if (fileInfo) {
        ...
        showFileInfo(dropZone, file) {
            let fileInfo = dropZone.querySelector('.drag-drop-file-info');
            if (!fileInfo) {
                fileInfo = document.createElement('div');
                fileInfo.className = 'drag-drop-file-info';
        ...
        // 顯示上傳進度
        showProgress(dropZone, progress) {
            let progressContainer = dropZone.querySelector('.upload-progress-container');
        ...
        // 顯示成功狀態
        showSuccess(dropZone) {
            dropZone.classList.add('success');
            dropZone.classList.remove('error', 'uploading');
            this.hideProgress(dropZone);
        },

        // 顯示錯誤狀態
        showError(dropZone, message) {
            dropZone.classList.add('error');
            dropZone.classList.remove('success', 'uploading');
            this.hideProgress(dropZone);
            ...
            setTimeout(() => {
                ...
                if (fileInfo && fileInfo.querySelector('.drag-drop-file-name')?.textContent === message) {
                    fileInfo.classList.remove('show');
                }
            }, 3000);
        },
        ...
        // 模擬上傳進度（用於演示）
        simulateUpload(dropZone, duration = 2000) {
            return new Promise((resolve) => {
                this.showUploading(dropZone);
                let progress = 0;
                const interval = setInterval(() => {
                    ...
                    if (progress >= 100) {
                        ...
                        this.showProgress(dropZone, 100);
                        setTimeout(() => {
                            this.showSuccess(dropZone);
                            resolve();
                        }, 300);
                    } else {
                        this.showProgress(dropZone, Math.round(progress));
                    }
                }, duration / 10);
            });
        }
...
:root {
    ...
    --primary-glow-light: rgba(245, 158, 11, 0.3);
    --primary-glow-subtle: rgba(245, 158, 11, 0.1);
    
    /* Nano 版專用黃色系 */
    --nano-primary: #FACC15;
    --nano-primary-dim: #cca400;
    --nano-glow: rgba(250, 204, 21, 0.6);
    --nano-glow-subtle: rgba(250, 204, 21, 0.2);
    ...
    --mobile-spacing: var(--spacing-md);
    --mobile-font-scale: 0.95;
}

/* 頁腳樣式 */
.footer{padding:20px;text-align:center;font-size:var(--font-sm);color:var(--text-placeholder);border-top:1px solid var(--border-primary);margin-top:auto;line-height:1.8}
.footer a{color:var(--primary-light);text-decoration:none;transition:var(--transition-normal);margin:0 4px}
.footer a:hover{text-decoration:underline;color:var(--primary)}
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;background:var(--gradient-bg);color:var(--text-primary);min-height:100vh}
.container{max-width:100%;margin:0;padding:0;height:100vh;display:flex;flex-direction:column}
.top-nav{background:var(--bg-panel-hover);backdrop-filter:var(--blur-sm);border-bottom:1px solid var(--border-primary);padding:var(--spacing-xl) 25px;display:flex;justify-content:space-between;align-items:center;flex-shrink:0}
.nav-left{display:flex;align-items:center;gap:20px}
.logo{color:var(--primary);font-size:var(--font-3xl);font-weight:800;text-shadow:var(--shadow-glow);display:flex;align-items:center;gap:10px}
.badge{background:var(--gradient-success);padding:4px 10px;border-radius:var(--radius-full);font-size:var(--font-xs);font-weight:600}
.nav-menu{display:flex;gap:10px;align-items:center}
.nav-btn{padding:8px 16px;background:var(--bg-panel-hover);border:1px solid var(--border-primary);border-radius:var(--radius-md);color:var(--text-muted);cursor:pointer;font-size:var(--font-lg);font-weight:600;transition:all var(--transition-normal);display:flex;align-items:center;gap:6px;text-decoration:none}
.nav-btn:hover{border-color:var(--primary);color:var(--text-primary)}
.nav-btn.active{background:var(--gradient-primary);color:var(--text-primary);border-color:var(--primary)}
.nav-btn.nano-btn:hover {border-color: var(--nano-primary); background: rgba(250, 204, 21, 0.1); color: var(--nano-primary); box-shadow: var(--shadow-glow-sm);}
.lang-btn{padding:6px 10px;background:var(--bg-panel-hover);border:1px solid var(--border-primary);border-radius:var(--radius-sm);color:var(--text-muted);cursor:pointer;font-size:var(--font-sm);margin-left:10px;position:relative}
.lang-dropdown{position:absolute;top:100%;right:0;background:var(--bg-glass);backdrop-filter:var(--blur-md);border:1px solid var(--border-primary);border-radius:var(--radius-md);padding:8px 0;min-width:140px;display:none;z-index:1000;box-shadow:var(--shadow-lg)}
.lang-dropdown.show{display:block}
.lang-option{padding:10px 16px;cursor:pointer;display:flex;align-items:center;gap:10px;transition:var(--transition-fast);color:var(--text-secondary)}
.lang-option:hover{background:var(--primary-glow-subtle);color:var(--primary)}
.lang-option.active{background:rgba(245,158,11,0.2);color:var(--primary)}
.lang-flag{font-size:16px}
.lang-name{font-size:var(--font-md);font-weight:500}
/* RTL Support */
[dir="rtl"]{direction:rtl;text-align:right}
...
[dir="rtl"] .left-panel{border-right:none;border-left:1px solid var(--border-primary)}
[dir="rtl"] .right-panel{border-left:none;border-right:1px solid var(--border-primary)}
[dir="rtl"] .gallery-actions{flex-direction:row-reverse}
[dir="rtl"] .gallery-meta{flex-direction:row-reverse}
.main-content{flex:1;display:flex;overflow:hidden}
.left-panel{width:320px;background:var(--bg-panel);border-right:1px solid var(--border-primary);overflow-y:auto;padding:var(--spacing-xl);flex-shrink:0}
.center-panel{flex:1;padding:var(--spacing-xl);overflow-y:auto}
.right-panel{width:380px;background:var(--bg-panel);border-left:1px solid var(--border-primary);overflow-y:auto;padding:var(--spacing-xl);flex-shrink:0}
/* 響應式設計 - 主頁面 */
@media(max-width:1024px){
    .main-content{flex-direction:column}
    .left-panel,.right-panel{width:100%;border:none;border-bottom:1px solid var(--border-primary)}
...
@media(max-width:768px){
    .top-nav{padding:var(--spacing-lg) var(--spacing-md)}
    .logo{font-size:var(--font-2xl)}
    .nav-btn{padding:6px 12px;font-size:var(--font-md)}
    .nav-btn span{display:none}
    .nav-btn i{margin:0}
    .left-panel,.right-panel{padding:var(--spacing-lg)}
    .gallery{grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:12px}
    .gallery-item img{height:200px}
    .gallery-info{padding:10px}
    .btn{padding:10px 16px;font-size:var(--font-md)}
    .form-group{margin-bottom:var(--spacing-md)}
    label{font-size:var(--font-sm)}
    input,textarea,select{padding:8px;font-size:var(--font-sm)}
}

@media(max-width:640px){
    .top-nav{flex-wrap:wrap;gap:10px;padding:var(--spacing-md)}
    ...
    .left-panel,.right-panel{padding:var(--spacing-md)}
    .gallery{grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:8px}
    .gallery-item img{height:150px}
    .gallery-info{padding:8px}
    .gallery-meta{flex-direction:column;align-items:flex-start;gap:3px}
    .model-badge,.seed-badge,.style-badge{font-size:10px;padding:2px 6px}
    .gallery-actions{flex-wrap:wrap}
    .action-btn{padding:4px 8px;font-size:var(--font-xs);min-width:calc(50% - 4px)}
    .btn{padding:8px 12px;font-size:var(--font-sm)}
    .history-header{flex-direction:column;gap:10px;padding:var(--spacing-lg)}
    .history-stats{flex-wrap:wrap;gap:10px;font-size:var(--font-md)}
    .modal-content img{max-width:95vw;max-height:80vh}
    .modal-close{top:10px;right:10px;font-size:24px}
}

@media(max-width:480px){
    .container{height:auto;min-height:100vh}
    .top-nav{padding:var(--spacing-sm)}
    .logo{font-size:var(--font-lg)}
    .nav-btn{padding:4px 8px;font-size:var(--font-xs)}
    .lang-btn{padding:4px 6px;font-size:var(--font-xs)}
    .lang-dropdown{min-width:120px}
    .left-panel,.right-panel{padding:var(--spacing-sm)}
    .gallery{grid-template-columns:repeat(2,1fr);gap:6px}
    .gallery-item img{height:120px}
    .gallery-info{padding:6px}
    .gallery-actions{gap:4px}
    .action-btn{padding:3px 6px;font-size:10px}
    .btn{padding:6px 10px;font-size:var(--font-xs)}
    .drag-drop-zone{padding:12px;min-height:80px}
    .drag-drop-zone .drag-icon{font-size:24px}
    .drag-drop-zone .drag-text{font-size:var(--font-xs)}
    .drag-drop-zone .drag-subtext{font-size:9px}
    .toast{padding:10px 15px;font-size:12px;top:10px;right:10px}
}
...
/* 觸控優化 */
@media(hover:none) and (pointer:coarse){
    .nav-btn,.btn,.action-btn,.lang-btn{min-height:var(--touch-target)}
    .gallery-item{min-height:var(--touch-target)}
    .drag-drop-zone{min-height:calc(var(--touch-target) * 2)}
    input,textarea,select{min-height:var(--touch-target)}
    select option{padding:10px}
}

/* 橫向模式優化 */
@media(max-height:600px) and (orientation:landscape){
    .top-nav{padding:var(--spacing-sm) var(--spacing-lg)}
...
label{display:block;margin-bottom:6px;font-weight:600;font-size:var(--font-md);color:var(--text-secondary)}
input,textarea,select{width:100%;padding:10px;background:var(--bg-input);border:1px solid var(--border-secondary);border-radius:var(--radius-md);color:var(--text-primary);font-size:var(--font-md);transition:all var(--transition-normal)}
input:focus,textarea:focus,select:focus{outline:none;border-color:var(--primary);box-shadow:0 0 0 3px var(--border-glow)}
select{background-color:var(--bg-tertiary)!important;color:var(--text-tertiary)!important;cursor:pointer}
.btn{padding:12px 24px;border:none;border-radius:var(--radius-md);font-size:var(--font-lg);font-weight:700;cursor:pointer;transition:all var(--transition-normal);display:inline-flex;align-items:center;gap:8px;justify-content:center;width:100%}
.btn-primary{background:var(--gradient-primary);color:var(--text-primary);box-shadow:0 4px 15px var(--primary-glow-light)}
.btn-primary:disabled{opacity:0.5;cursor:not-allowed}
.gallery{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:20px}
.gallery-item{background:rgba(0,0,0,0.4);border-radius:var(--radius-xl);overflow:hidden;border:1px solid var(--border-primary);transition:all var(--transition-normal)}
...
.model-badge,.seed-badge,.style-badge{padding:4px 10px;border-radius:var(--radius-sm);font-size:var(--font-xs);font-weight:600;background:var(--bg-panel-hover)}
.gallery-actions{display:flex;gap:8px;margin-top:10px}
.action-btn{padding:6px 12px;background:var(--bg-panel-hover);border:1px solid var(--border-secondary);border-radius:var(--radius-sm);font-size:var(--font-sm);color:var(--text-primary);cursor:pointer;flex:1}
.action-btn:hover{background:var(--bg-panel)}
/* 加載狀態優化 - 主頁面 */
.loading{text-align:center;padding:60px 20px;color:var(--text-muted)}
.spinner{border:3px solid var(--border-primary);border-top:3px solid var(--primary);border-radius:50%;width:40px;height:40px;animation:spin 1s linear infinite;margin:0 auto 15px;box-shadow:0 0 15px var(--primary-glow-subtle)}
@keyframes spin{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}
.loading-container{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px}
.loading-message{font-size:var(--font-lg);color:var(--text-secondary);font-weight:500}
.loading-submessage{font-size:var(--font-sm);color:var(--text-muted)}
.loading-dots{display:inline-flex;gap:4px;margin-left:8px}
.loading-dot{width:6px;height:6px;background:var(--primary);border-radius:50%;animation:dotPulse 1.4s ease-in-out infinite}
.loading-dot:nth-child(1){animation-delay:0s}
.loading-dot:nth-child(2){animation-delay:0.2s}
.loading-dot:nth-child(3){animation-delay:0.4s}
@keyframes dotPulse{0%,80%,100%{transform:scale(0.6);opacity:0.5}40%{transform:scale(1);opacity:1}}
.history-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding:20px;background:var(--bg-panel);border-radius:var(--radius-xl)}
.history-stats{display:flex;gap:20px;font-size:var(--font-lg)}
.modal{display:none;position:fixed;z-index:1000;left:0;top:0;width:100%;height:100%;background:var(--bg-overlay);align-items:center;justify-content:center}
.modal.show{display:flex}
.modal-content img{max-width:90vw;max-height:90vh;border-radius:var(--radius-md)}
.modal-close{position:absolute;top:20px;right:20px;color:var(--text-primary);font-size:32px;cursor:pointer}
@keyframes pulse { 0% { opacity: 1; } 50% { opacity: 0.5; } 100% { opacity: 1; } }
/* 拖放區域樣式 - 主頁面版本 */
.drag-drop-zone {
...
.drag-drop-zone.uploading .drag-icon {
...
.drag-drop-zone.success .drag-icon {
    color: var(--success);
}
...
.drag-drop-zone.error .drag-icon {
    color: var(--danger);
}
...
.drag-drop-preview .preview-remove {
    position: absolute;
    top: 5px;
    right: 5px;
    width: 24px;
...
.drag-drop-preview .preview-remove:hover {
...
.drag-drop-zone:hover .drag-drop-hint {
    opacity: 1;
}
...
/* Nano 版拖放區域優化 */
.nano-drag-drop-zone.uploading {
    pointer-events: none;
    opacity: 0.7;
}
.nano-drag-drop-zone.uploading .drag-icon {
    animation: spin 1s linear infinite;
}
.nano-drag-drop-zone.success {
    border-color: var(--nano-primary);
    background: rgba(250, 204, 21, 0.1);
}
.nano-drag-drop-zone.success .drag-icon {
    color: var(--nano-primary);
}
.nano-drag-drop-zone.error {
    border-color: var(--danger);
    background: rgba(239, 68, 68, 0.1);
}
.nano-drag-drop-zone.error .drag-icon {
    color: var(--danger);
}
.nano-drag-drop-zone.drag-over {
    animation: dragPulse 1s ease-in-out infinite;
...
.nano-drag-drop-preview.show {
    display: flex;
}
.nano-drag-drop-preview img {
    max-width: 100%;
    max-height: 120px;
    border-radius: var(--radius-md);
    object-fit: contain;
}
.nano-drag-drop-preview .preview-remove {
    position: absolute;
    top: 5px;
    right: 5px;
    width: 24px;
...
.nano-drag-drop-preview .preview-remove:hover {
    background: var(--danger-dark);
    transform: scale(1.1);
}
...
.quick-style-preset:hover {
    border-color: var(--primary);
    background: var(--primary-glow-subtle);
    color: var(--text-primary);
}

.quick-style-preset.active {
    background: var(--gradient-primary);
    border-color: var(--primary);
    color: var(--text-primary);
}
...
.quick-size-preset:hover {
    border-color: var(--primary);
    background: var(--primary-glow-subtle);
    color: var(--text-primary);
}

.quick-size-preset.active {
    background: var(--gradient-primary);
    border-color: var(--primary);
    color: var(--text-primary);
}
...
.quick-quality-mode:hover {
    border-color: var(--primary);
    background: var(--primary-glow-subtle);
    color: var(--text-primary);
}

.quick-quality-mode.active {
    background: var(--gradient-primary);
    border-color: var(--primary);
    color: var(--text-primary);
}
...
.quick-toolbar-btn.danger:hover {
    border-color: var(--danger);
    background: rgba(239, 68, 68, 0.1);
    color: var(--danger);
}
...
.quick-style-preset:focus-visible,
.quick-size-preset:focus-visible,
.quick-quality-mode:focus-visible,
.quick-toolbar-btn:focus-visible,
.ratio-btn:focus-visible {
    outline: 2px solid var(--primary);
    outline-offset: 2px;
    box-shadow: 0 0 0 4px var(--primary-glow-subtle);
}
...
.modal[role="dialog"][aria-hidden="false"] {
    display: flex;
}
...
.progress-bar[role="progressbar"]::-webkit-progress-bar {
    background: var(--border-primary);
}

.progress-bar[role="progressbar"]::-webkit-progress-value {
    background: var(--gradient-primary);
}

.progress-bar[role="progressbar"]::-moz-progress-bar {
    background: var(--gradient-primary);
}
...
/* Nano 頁面無障礙優化 */
.nano-drag-drop-zone[aria-dropeffect="copy"] {
    border-style: dashed;
    border-width: 2px;
}

.nano-drag-drop-zone[aria-dropeffect="copy"]:focus {
...
/* 歷史記錄無障礙 */
.history-item[role="button"] {
    cursor: pointer;
}

.history-item[role="button"]:focus {
    outline: 2px solid var(--primary);
    outline-offset: 2px;
}
...
.gallery-item[role="button"]:focus {
    outline: 2px solid var(--primary);
    outline-offset: 2px;
}
...
/* 語言選擇器無障礙 */
.lang-dropdown[role="listbox"] {
    max-height: 300px;
    overflow-y: auto;
}

.lang-option[role="option"] {
    padding: 10px 16px;
    cursor: pointer;
    display: flex;
    align-items: center;
...
.lang-option[role="option"][aria-selected="true"] {
    background: rgba(245, 158, 11, 0.2);
    color: var(--primary);
}

.lang-option[role="option"]:hover,
.lang-option[role="option"]:focus {
    background: var(--primary-glow-subtle);
    color: var(--primary);
    outline: none;
}
...
/* 風格選擇器無障礙 */
.style-selector[role="combobox"] {
    position: relative;
}

.recent-style-item[role="button"] {
    cursor: pointer;
}

.recent-style-item[role="button"]:focus {
    outline: 2px solid var(--primary);
    outline-offset: 2px;
}

/* 快捷操作面板無障礙 */
.quick-actions-panel[role="region"] {
    border: 1px solid var(--border-primary);
}

.quick-actions-panel[role="region"] > *:first-child {
    margin-top: 0;
}

.quick-actions-panel[role="region"] > *:last-child {
    margin-bottom: 0;
}

/* 拖放區域無障礙 */
.drag-drop-zone[role="button"] {
    cursor: pointer;
}

.drag-drop-zone[role="button"]:focus {
    outline: 2px solid var(--primary);
    outline-offset: 2px;
}

.drag-drop-zone[aria-label] {
    position: relative;
}
...
.style-selector-search input:focus {
    outline: none;
    border-color: var(--primary);
    box-shadow: 0 0 0 3px var(--border-glow);
}

.style-selector-search-icon {
    position: absolute;
    left: 10px;
    top: 50%;
...
.style-selector-clear:hover {
    color: var(--text-primary);
}

.style-selector-clear.show {
    display: block;
}
...
.style-selector option:hover {
    background: var(--primary-glow-subtle);
}

.style-selector option:checked {
    background: var(--gradient-primary);
    color: var(--text-primary);
    font-weight: 600;
}
...
.recent-style-item:hover {
    border-color: var(--primary);
    background: var(--primary-glow-subtle);
    color: var(--text-primary);
}

.recent-style-item.active {
    background: var(--gradient-primary);
    border-color: var(--primary);
    color: var(--text-primary);
}
...
.style-preview-search {
    margin-bottom: 16px;
}

.style-preview-search input {
    width: 100%;
...
<body>
<!-- 無障礙跳過導航鏈接 -->
<a href="#main-content" class="skip-link" data-t="skip_to_main">跳過導航，直接前往主要內容</a>
<a href="#generateForm" class="skip-link" style="left: 200px;" data-t="skip_to_form">跳過導航，直接前往生成表單</a>
...
<div class="form-group">
    ...
    <div class="style-selector-wrapper">
        <div class="style-selector-search">
            <span class="style-selector-search-icon">🔍</span>
            <input type="text" id="styleSearch" placeholder="搜尋風格..." autocomplete="off">
            <button type="button" class="style-selector-clear" id="styleSearchClear">✕</button>
        </div>
        <select id="style" class="style-selector">${styleOptionsHTML}</select>
        <div class="recent-styles-header">
            <span class="recent-styles-title">最近使用</span>
            <button type="button" class="recent-styles-clear" id="recentStylesClear">清除</button>
        </div>
        <div class="recent-styles" id="recentStyles">
            <!-- JS will populate this -->
        </div>
    </div>
</div>
...
<div class="quick-actions-panel" id="quickActionsPanel">
    <div class="quick-actions-header">
        <div class="quick-actions-title">
            <i>⚡</i>
            <span data-t="quick_actions_title">快捷操作</span>
        </div>
        <button class="quick-actions-toggle" id="quickActionsToggle" data-t="collapse">收起</button>
    </div>
    
    <div class="quick-actions-content" id="quickActionsContent">
        <!-- 快捷風格預設 -->
        <div class="quick-actions-group">
            <div class="quick-actions-group-title" data-t="quick_styles">熱門風格</div>
            <div class="quick-style-presets" id="quickStylePresets">
                <button class="quick-style-preset" data-style="anime">🎌 動漫</button>
                <button class="quick-style-preset" data-style="photorealistic">📷 寫實</button>
                <button class="quick-style-preset" data-style="cyberpunk">🌆 賽博龐克</button>
                <button class="quick-style-preset" data-style="watercolor">🎨 水彩</button>
                <button class="quick-style-preset" data-style="3d-render">🎲 3D</button>
                <button class="quick-style-preset" data-style="pixel-art">👾 像素</button>
            </div>
        </div>
        
        <!-- 快捷尺寸預設 -->
        <div class="quick-actions-group">
            <div class="quick-actions-group-title" data-t="quick_sizes">常用尺寸</div>
            <div class="quick-size-presets" id="quickSizePresets">
                <button class="quick-size-preset" data-width="1024" data-height="1024">
                    <span class="quick-size-icon">⬜</span>
                    <span>1:1</span>
                </button>
                <button class="quick-size-preset" data-width="1024" data-height="768">
                    <span class="quick-size-icon">▭</span>
                    <span>4:3</span>
                </button>
                <button class="quick-size-preset" data-width="768" data-height="1024">
                    <span class="quick-size-icon">▯</span>
                    <span>3:4</span>
                </button>
                <button class="quick-size-preset" data-width="1280" data-height="720">
                    <span class="quick-size-icon">▬</span>
                    <span>16:9</span>
                </button>
                <button class="quick-size-preset" data-width="720" data-height="1280">
                    <span class="quick-size-icon">▮</span>
                    <span>9:16</span>
                </button>
            </div>
        ...
        <!-- 快捷質量模式 -->
        <div class="quick-actions-group">
            <div class="quick-actions-group-title" data-t="quick_quality">質量模式</div>
            <div class="quick-quality-modes" id="quickQualityModes">
                <button class="quick-quality-mode" data-quality="standard" data-t="quality_standard">標準</button>
                <button class="quick-quality-mode" data-quality="high" data-t="quality_high">高清</button>
                <button class="quick-quality-mode" data-quality="ultra" data-t="quality_ultra">超清</button>
            </div>
        </div>
        
        <!-- 快捷工具欄 -->
        <div class="quick-actions-toolbar">
            <button class="quick-toolbar-btn" id="quickRandomPrompt" data-t="random_prompt">
                <i>🎲</i>
                <span>隨機提示詞</span>
            </button>
            <button class="quick-toolbar-btn" id="quickClearAll" data-t="clear_all">
                <i>🗑️</i>
                <span>清空全部</span>
            </button>
            <button class="quick-toolbar-btn danger" id="quickResetSettings" data-t="reset_settings">
                <i>🔄</i>
                <span>重置設置</span>
            </button>
        </div>
...
const StyleSelector = {
    ...
    loadRecentStyles() {
        try {
            ...
            if (stored) {
                this.recentStyles = JSON.parse(stored);
            }
        } catch (e) {
    ...
    saveRecentStyles() {
        try {
            localStorage.setItem(this.storageKey, JSON.stringify(this.recentStyles));
        } catch (e) {
            console.error('Failed to save recent styles:', e);
        }
    },
    
    // 添加風格到最近使用
    addRecentStyle(styleKey) {
        if (styleKey === 'none') return;
        
        // 移除已存在的相同風格
        this.recentStyles = this.recentStyles.filter(s => s.key !== styleKey);
        
        // 添加到開頭
        const styleConfig = STYLE_PRESETS[styleKey];
        if (styleConfig) {
            let styleName = typeof styleConfig.name === 'object' ? (styleConfig.name[curLang] || styleConfig.name.zh || styleConfig.name) : styleConfig.name;
            this.recentStyles.unshift({
                key: styleKey,
                name: styleName,
            ...
            // 限制數量
            if (this.recentStyles.length > this.maxRecentStyles) {
                this.recentStyles = this.recentStyles.slice(0, this.maxRecentStyles);
            }
            
            this.saveRecentStyles();
            this.renderRecentStyles();
        }
    },
    ...
    // 渲染最近使用的風格
    renderRecentStyles() {
        const container = document.getElementById('recentStyles');
        if (!container) return;
        
        if (this.recentStyles.length === 0) {
            container.innerHTML = '<span style="color: var(--text-muted); font-size: var(--font-xs);">暫無最近使用</span>';
            return;
        }
        ...
        container.querySelectorAll('.recent-style-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                if (styleSelect) {
                    ...
                    this.updateActiveRecentStyle(styleKey);
                }
            });
        });
        ...
        if (currentStyle) {
            this.updateActiveRecentStyle(currentStyle);
        }
    },
    
    // 更新最近使用風格的激活狀態
    updateActiveRecentStyle(styleKey) {
        document.querySelectorAll('.recent-style-item').forEach(item => {
            if (item.dataset.style === styleKey) {
    ...
    searchStyles(query) {
        ...
        options.forEach(option => {
            ...
            if (text.includes(lowerQuery) || value.includes(lowerQuery)) {
        ...
        // 隱藏空的 optgroup
        const optgroups = styleSelect.querySelectorAll('optgroup');
    ...
    bindEvents() {
        ...
        // 清除最近使用按鈕
        const clearRecentBtn = document.getElementById('recentStylesClear');
        if (clearRecentBtn) {
            clearRecentBtn.addEventListener('click', () => {
                if (confirm('確定要清除最近使用的風格嗎？')) {
                    this.clearRecentStyles();
                }
            });
        }
        ...
        if (styleSelect) {
            styleSelect.addEventListener('change', (e) => {
                this.addRecentStyle(e.target.value);
                this.updateActiveRecentStyle(e.target.value);
            });
        }
    }
};

// 初始化風格選擇器 - 已合併到主 DOMContentLoaded 監聽器
    ...
    async showPreview(dropZone, file) {
        try {
            ...
            reader.onload = (e) => {
                ...
                // 創建或更新預覽元素
                let preview = dropZone.querySelector('.drag-drop-preview');
                if (!preview) {
                    preview = document.createElement('div');
...
const QuickActionsPanel = {
    ...
    randomPrompts: [
        ...
        'A underwater kingdom with colorful coral and fish',
        'A steampunk airship flying through clouds',
        'A mystical library with floating books',
        'A cherry blossom tree in full bloom with a temple',
        'A cyberpunk street vendor selling neon food',
    ...
    bindEvents() {
        ...
        // 快捷風格預設
        const stylePresets = document.querySelectorAll('.quick-style-preset');
        stylePresets.forEach(btn => {
        ...
        // 快捷尺寸預設
        const sizePresets = document.querySelectorAll('.quick-size-preset');
        sizePresets.forEach(btn => {
            btn.addEventListener('click', () => {
                const width = parseInt(btn.dataset.width);
                const height = parseInt(btn.dataset.height);
                this.selectSize(width, height);
            });
        });
        
        // 快捷質量模式
        const qualityModes = document.querySelectorAll('.quick-quality-mode');
        qualityModes.forEach(btn => {
            btn.addEventListener('click', () => {
        ...
        // 隨機提示詞
        const randomPromptBtn = document.getElementById('quickRandomPrompt');
        if (randomPromptBtn) {
        ...
        // 清空全部
        const clearAllBtn = document.getElementById('quickClearAll');
        if (clearAllBtn) {
        ...
        // 重置設置
        const resetSettingsBtn = document.getElementById('quickResetSettings');
        if (resetSettingsBtn) {
    ...
    // 加載保存的狀態
    loadSavedState() {
        const collapsed = localStorage.getItem('quickActionsCollapsed');
        if (collapsed === 'true') {
            const content = document.getElementById('quickActionsContent');
            const toggleBtn = document.getElementById('quickActionsToggle');
    ...
    selectStyle(styleKey) {
        ...
        if (styleSelect) {
            ...
            // 更新激活狀態
            document.querySelectorAll('.quick-style-preset').forEach(btn => {
    ...
    selectSize(width, height) {
        ...
        if (widthInput && heightInput) {
            ...
            // 更新激活狀態
            document.querySelectorAll('.quick-size-preset').forEach(btn => {
    ...
    selectQuality(quality) {
        ...
        if (stepsInput && guidanceInput) {
            ...
            switch (quality) {
                ...
                case 'high':
                    stepsInput.value = 30;
                    guidanceInput.value = 8.0;
                    break;
                case 'ultra':
                    stepsInput.value = 50;
                    guidanceInput.value = 8.5;
                    break;
            }
            
            // 更新激活狀態
            document.querySelectorAll('.quick-quality-mode').forEach(btn => {
    ...
    generateRandomPrompt() {
        const prompt = this.randomPrompts[Math.floor(Math.random() * this.randomPrompts.length)];
        const promptInput = document.getElementById('prompt');
        ...
        if (promptInput) {
            ...
            // 添加動畫效果
            promptInput.style.transition = 'background-color 0.3s';
            promptInput.style.backgroundColor = 'rgba(245, 158, 11, 0.2)';
            setTimeout(() => {
    ...
    clearAll() {
        if (confirm('確定要清空所有輸入嗎？')) {
            ...
            if (negativePromptInput) negativePromptInput.value = 'nsfw, ugly, text, watermark, low quality, bad anatomy, distortion, blurry';
            if (referenceImagesInput) referenceImagesInput.value = '';
            
            // 清除拖放區域
            const dropZone = document.getElementById('imageDropZone');
            if (dropZone) {
                DragDropHandler.clearPreview(dropZone);
            }
        }
    },
    ...
    resetSettings() {
        if (confirm('確定要重置所有設置為默認值嗎？')) {
            ...
            if (guidanceInput) guidanceInput.value = 7.5;
            ...
            document.querySelectorAll('.quick-size-preset').forEach(btn => {
                const btnWidth = parseInt(btn.dataset.width);
                const btnHeight = parseInt(btn.dataset.height);
                if (btnWidth === 1024 && btnHeight === 1024) {
                    btn.classList.add('active');
                } else {
                    btn.classList.remove('active');
                }
    ...
    updateActiveStates() {
        ...
        if (styleSelect) {
            document.querySelectorAll('.quick-style-preset').forEach(btn => {
                if (btn.dataset.style === styleSelect.value) {
                    btn.classList.add('active');
                } else {
                    btn.classList.remove('active');
                }
...
// 初始化快捷操作面板 - 已合併到主 DOMContentLoaded 監聽器
...
const KeyboardShortcuts = {
    ...
    shortcuts: {
        ...
        'Ctrl+R': {
            ...
            description: '隨機提示詞',
            handler: () => {
                QuickActionsPanel.generateRandomPrompt();
            }
        },
        'Cmd+R': {
            action: 'randomPrompt',
            description: '隨機提示詞 (Mac)',
            handler: () => {
                QuickActionsPanel.generateRandomPrompt();
            }
        },
        ...
        'Ctrl+K': {
            ...
            description: '切換快捷操作面板',
            handler: () => {
                QuickActionsPanel.togglePanel();
            }
        },
        'Cmd+K': {
            action: 'toggleQuickActions',
            description: '切換快捷操作面板 (Mac)',
            handler: () => {
                QuickActionsPanel.togglePanel();
            }
        },
    ...
    bindEvents() {
        document.addEventListener('keydown', (e) => {
            ...
            if (shortcutConfig) {
                // 如果在輸入框中，只允許某些快捷鍵
                if (isInputFocused && !['Escape', 'Ctrl+Enter', 'Cmd+Enter'].includes(shortcut)) {
                    return;
                }
    ...
    createHelpModal() {
        // 檢查是否已存在
        if (document.getElementById('keyboardShortcutsHelp')) return;
    ...
    generateShortcutsList() {
        ...
        Object.entries(this.shortcuts).forEach(([key, config]) => {
            if (!groupedShortcuts[config.action]) {
                groupedShortcuts[config.action] = [];
            }
            groupedShortcuts[config.action].push({ key, ...config });
        });
...
// 初始化鍵盤快捷鍵 - 已合併到主 DOMContentLoaded 監聽器
...
const AccessibilityManager = {
    ...
    addAriaLabels() {
        ...
        const generateBtn = document.getElementById('generateBtn');
        if (generateBtn) {
            generateBtn.setAttribute('aria-label', '開始生成圖片');
            generateBtn.setAttribute('aria-describedby', 'generateBtnDesc');
        }
        
        // 為提示詞輸入框添加 ARIA 標籤
        const promptInput = document.getElementById('prompt');
        if (promptInput) {
            promptInput.setAttribute('aria-label', '輸入圖片生成提示詞');
            promptInput.setAttribute('aria-required', 'true');
        }
        
        // 為風格選擇器添加 ARIA 標籤
        const styleSelect = document.getElementById('style');
        if (styleSelect) {
            styleSelect.setAttribute('aria-label', '選擇藝術風格');
        }
        
        // 為尺寸選擇器添加 ARIA 標籤
        const sizeSelect = document.getElementById('size');
        if (sizeSelect) {
            sizeSelect.setAttribute('aria-label', '選擇圖片尺寸');
        }
        
        // 為質量模式選擇器添加 ARIA 標籤
        const qualitySelect = document.getElementById('qualityMode');
        if (qualitySelect) {
            qualitySelect.setAttribute('aria-label', '選擇質量模式');
        }
        
        // 為模型選擇器添加 ARIA 標籤
        const modelSelect = document.getElementById('model');
        if (modelSelect) {
            modelSelect.setAttribute('aria-label', '選擇 AI 模型');
        }
        
        // 為語言切換按鈕添加 ARIA 標籤
        const langBtn = document.getElementById('langSwitch');
        if (langBtn) {
            langBtn.setAttribute('aria-label', '切換語言');
        ...
        if (form) {
            form.setAttribute('aria-label', '圖片生成表單');
        }
        
        // 為拖放區域添加 ARIA 標籤
        const dropZone = document.getElementById('imageDropZone');
        if (dropZone) {
            dropZone.setAttribute('aria-label', '拖放圖片到這裡上傳');
        ...
        if (quickActionsPanel) {
            quickActionsPanel.setAttribute('aria-label', '快捷操作面板');
        }
        ...
        // 為快捷尺寸預設按鈕添加 ARIA 標籤
        const quickSizePresets = document.querySelectorAll('.quick-size-preset');
        ...
        // 為歷史記錄按鈕添加 ARIA 標籤
        const historyBtn = document.querySelector('[data-page="history"]');
        if (historyBtn) {
    ...
    updateAriaStates() {
        ...
        if (quickActionsContent && quickActionsToggle) {
            const isCollapsed = quickActionsContent.style.display === 'none';
            quickActionsToggle.setAttribute('aria-expanded', (!isCollapsed).toString());
        }
    },
    ...
    announceSuccess(message) {
        ...
        // 創建或更新成功區域
        let successRegion = document.getElementById('success-region');
        if (!successRegion) {
    ...
    // 為加載狀態添加 ARIA 標籤
    announceLoading(isLoading, message = '正在加載...') {
        let loadingRegion = document.getElementById('loading-region');
    ...
    // 為進度條添加 ARIA 標籤
    updateProgress(current, total, label = '生成進度') {
        let progressRegion = document.getElementById('progress-region');
        ...
        progressRegion.setAttribute('aria-valuenow', current.toString());
    ...
    initialize() {
        ...
        interactiveElements.forEach(el => {
            el.addEventListener('keydown', (e) => {
                // 支持 Enter 和 Space 鍵激活按鈕
                if ((e.key === 'Enter' || e.key === ' ') && (el.tagName === 'BUTTON' || el.getAttribute('role') === 'button')) {
                    e.preventDefault();
                    el.click();
                }
            });
        ...
        if (quickActionsToggle) {
            quickActionsToggle.setAttribute('tabindex', '0');
            quickActionsToggle.addEventListener('keydown', (e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    QuickActionsPanel.togglePanel();
                }
        ...
        // 為風格預覽模態框添加無障礙支持
        const stylePreviewModal = document.querySelector('.style-preview-modal');
        if (stylePreviewModal) {
            stylePreviewModal.setAttribute('role', 'dialog');
            stylePreviewModal.setAttribute('aria-modal', 'true');
            stylePreviewModal.setAttribute('aria-labelledby', 'style-preview-title');
            
            const closeBtn = stylePreviewModal.querySelector('.style-preview-close');
            if (closeBtn) {
        ...
        if (shortcutsModal) {
            ...
            shortcutsModal.setAttribute('aria-labelledby', 'shortcuts-modal-title');
            
            const closeBtn = shortcutsModal.querySelector('.shortcuts-modal-close');
            if (closeBtn) {
                closeBtn.setAttribute('aria-label', '關閉快捷鍵幫助');
            }
        }
        ...
        if (historyPage) {
            historyPage.setAttribute('aria-label', '歷史記錄頁面');
        }
        ...
        if (resultArea) {
            ...
            resultArea.setAttribute('aria-label', '生成結果');
        }
        ...
        if (form) {
            form.addEventListener('submit', (e) => {
                ...
                if (!promptInput || !promptInput.value.trim()) {
                    e.preventDefault();
                    this.announceError('請輸入提示詞');
                    if (promptInput) {
                        promptInput.setAttribute('aria-invalid', 'true');
                        promptInput.setAttribute('aria-describedby', 'prompt-error');
                        
                        let errorMsg = document.getElementById('prompt-error');
                        if (!errorMsg) {
                            errorMsg = document.createElement('div');
                            errorMsg.id = 'prompt-error';
                            errorMsg.className = 'sr-only';
                            promptInput.parentNode.appendChild(errorMsg);
                        }
                        errorMsg.textContent = '請輸入提示詞';
                        promptInput.focus();
                    }
                } else if (promptInput) {
        ...
        const seedInput = document.getElementById('seed');
        const seedToggleBtn = document.getElementById('seedToggleBtn');
        if (seedInput && seedToggleBtn) {
            seedToggleBtn.setAttribute('aria-label', '切換隨機種子碼');
            seedToggleBtn.setAttribute('aria-pressed', 'false');
            
            seedToggleBtn.addEventListener('click', () => {
                const isRandom = seedInput.disabled;
                seedToggleBtn.setAttribute('aria-pressed', (!isRandom).toString());
                if (!isRandom) {
                    this.announce('已啟用隨機種子碼');
                } else {
                    this.announce('已啟用自定義種子碼');
                }
            });
        }
        ...
        if (autoOptimizeCheckbox) {
            autoOptimizeCheckbox.addEventListener('change', () => {
                if (autoOptimizeCheckbox.checked) {
                    this.announceSuccess('已啟用自動優化');
                } else {
                    this.announce('已停用自動優化');
                }
            });
        }
        ...
        if (nsfwToggle) {
            nsfwToggle.addEventListener('change', () => {
                if (nsfwToggle.checked) {
                    this.announceWarning('已啟用成人內容限制解除');
                } else {
                    this.announce('已啟用成人內容限制');
                }
            });
        }
        ...
        if (advancedParams) {
            advancedParams.setAttribute('aria-expanded', 'false');
        }
        
        // 為批量生成區域添加無障礙支持
        const batchGroup = document.getElementById('batchGroup');
        if (batchGroup) {
            batchGroup.setAttribute('aria-expanded', 'false');
        }
        
        // 為風格搜索框添加無障礙支持
        const styleSearch = document.getElementById('styleSearch');
        if (styleSearch) {
            styleSearch.setAttribute('aria-label', '搜索風格');
            styleSearch.setAttribute('aria-autocomplete', 'list');
            styleSearch.setAttribute('aria-controls', 'style');
        }
        
        // 為最近使用的風格區域添加無障礙支持
        const recentStyles = document.getElementById('recentStyles');
        if (recentStyles) {
            recentStyles.setAttribute('aria-label', '最近使用的風格');
            recentStyles.setAttribute('role', 'listbox');
        }
        
        // 為清除最近使用按鈕添加無障礙支持
        const recentStylesClear = document.getElementById('recentStylesClear');
        if (recentStylesClear) {
            recentStylesClear.setAttribute('aria-label', '清除最近使用的風格');
        }
        
        // 為清除搜索按鈕添加無障礙支持
        const styleSearchClear = document.getElementById('styleSearchClear');
        if (styleSearchClear) {
            styleSearchClear.setAttribute('aria-label', '清除搜索');
        }
        
        // 為隨機提示詞按鈕添加無障礙支持
        const randomPromptBtn = document.getElementById('quickRandomPrompt');
        if (randomPromptBtn) {
            randomPromptBtn.setAttribute('aria-label', '生成隨機提示詞');
        }
        
        // 為清空全部按鈕添加無障礙支持
        const clearAllBtn = document.getElementById('quickClearAll');
        if (clearAllBtn) {
            clearAllBtn.setAttribute('aria-label', '清空所有輸入');
        }
        
        // 為重置設置按鈕添加無障礙支持
        const resetSettingsBtn = document.getElementById('quickResetSettings');
        if (resetSettingsBtn) {
            resetSettingsBtn.setAttribute('aria-label', '重置所有設置為默認值');
        }
        
        // 為生成提示詞按鈕添加無障礙支持
        const generatePromptBtn = document.getElementById('generatePromptBtn');
        if (generatePromptBtn) {
            generatePromptBtn.setAttribute('aria-label', '生成提示詞');
        }
        
        // 為應用提示詞按鈕添加無障礙支持
        const applyPromptBtn = document.getElementById('applyPromptBtn');
        if (applyPromptBtn) {
            applyPromptBtn.setAttribute('aria-label', '應用生成的提示詞');
        }
        
        // 為提示詞輸入框添加無障礙支持
        const promptInput = document.getElementById('promptInput');
        if (promptInput) {
            promptInput.setAttribute('aria-label', '輸入提示詞生成器的輸入');
        }
        
        // 為提示詞圖片上傳區域添加無障礙支持
        const promptImageDropZone = document.getElementById('promptImageDropZone');
        if (promptImageDropZone) {
            promptImageDropZone.setAttribute('aria-label', '拖放圖片到這裡上傳用於生成提示詞');
            promptImageDropZone.setAttribute('role', 'button');
            promptImageDropZone.setAttribute('tabindex', '0');
        }
        
        // 為提示詞圖片預覽添加無障礙支持
        const promptImagePreview = document.getElementById('promptImagePreview');
        if (promptImagePreview) {
            promptImagePreview.setAttribute('role', 'img');
            promptImagePreview.setAttribute('aria-label', '上傳的圖片預覽');
        }
        
        // 為清除提示詞圖片按鈕添加無障礙支持
        const promptImageClearBtn = document.getElementById('promptImageClearBtn');
        if (promptImageClearBtn) {
            promptImageClearBtn.setAttribute('aria-label', '清除上傳的圖片');
        }
        
        // 為提示詞生成器狀態添加無障礙支持
        const promptGeneratorStatus = document.getElementById('promptGeneratorStatus');
        if (promptGeneratorStatus) {
            promptGeneratorStatus.setAttribute('role', 'status');
            promptGeneratorStatus.setAttribute('aria-live', 'polite');
        }
        ...
        const historyCount = document.getElementById('historyCount');
        if (historyCount) {
            historyCount.setAttribute('aria-label', '歷史記錄數量');
        }
        ...
        // 為所有鏈接添加描述性文本（如果只有圖片）
        const linksWithOnlyImages = document.querySelectorAll('a:has(> img:only-child)');
        linksWithOnlyImages.forEach(link => {
        ...
        inputsWithoutLabels.forEach(input => {
            input.setAttribute('aria-label', '輸入框');
        });
        
        // 為所有按鈕添加 aria-label（如果沒有文本內容）
        ...
        // 初始化完成公告
        this.announceSuccess('無障礙功能已初始化完成');
    },
